...
public class GifPoolRunner : MonoBehaviour {
    ...
    void Update() {
        ...
        if (GameControl.instance.gameOver == false && CoinCollect.isTriggeredForSonic) {

            if (!columns[currentColumn].gameObject.activeSelf)
                columns[currentColumn].gameObject.SetActive(true);

            //...then set the current column to that position.
            if (CoinCollect.isTriggeredForSonic)
            ...
            CoinCollect.isTriggeredForSonic = false;
